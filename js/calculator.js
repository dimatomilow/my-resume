...
calculatorForm.addEventListener("submit", function (_event) {
...
function calculateSum() {
 ...
 //Values
  ...
  const websiteReceptiontValue = convertReceptionOptionToPrice(websiteReception.value);


  const totalSum = websiteTypeValue + technologiesValue + websiteCartValue + websiteReceptiontValue;
...
function getTechnologiesSum(technologiesArr) {
  ...
  technologiesArr.forEach(function (tech) {
   totalSun = totalSun + extractPriceFromValue (tech.value)
  })
  return totalSun;
}
